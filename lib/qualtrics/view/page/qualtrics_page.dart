...
class QualtricsPage extends StatefulWidget {
  const QualtricsPage({super.key});

  @override
  State<QualtricsPage> createState() => _QualtricsPageState();
}

class _QualtricsPageState extends State<QualtricsPage>
    with AutomaticKeepAliveClientMixin<QualtricsPage> {
  @override
...
class _InterceptDetailsState extends State<InterceptDetails> {
  ...
  Future<void> _initializeProject() async {
    var initResults =
        await qualtrics.initializeProject(_brandIdController.text, _zoneIdController.text);
  ...
  Future<void> _setString() async {
    await qualtrics.setString(
        _stringCustomPropertyKeyController.text, _stringCustomPropertyValueController.text);
  }
